...
export default {
  ...
  methods: {
    async initChart() {
      ...
      // res = await res.json()
      const res = await axios.get('http://localhost:8099/static/map/china.json')
      // console.log(res)
      ...
      this.chartInstance.on('click', async (e) => {
        ...
        if (!this.mapData[provinceInfo.key] ) {
          ...
          const res = await axios.get('http://localhost:8099' + provinceInfo.path);
